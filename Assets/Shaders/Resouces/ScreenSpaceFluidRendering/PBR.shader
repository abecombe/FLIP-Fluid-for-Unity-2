Shader "ScreenSpaceFluidRendering/PBR"
{
    ...
    inline float3 GetEyeSpacePos(float2 screen_space_pos)
    {
        const float depth = tex2D(_MainTex, screen_space_pos).r * _FarClipPlane;
        return float3(depth * (screen_space_pos * 2.0 - 1.0) * _ClipToViewConst, -depth);
    }

    inline void CalcEyeSpacePosNormViewVec(in float2 screen_space_pos, out float3 eye_space_pos, out float3 eye_space_norm, out float3 eye_space_view_vec)
    {
        eye_space_pos = GetEyeSpacePos(screen_space_pos);

        const float2 inv_res = _MainTex_TexelSize.xy;
        float3 ddx = GetEyeSpacePos(screen_space_pos + float2(inv_res.x, 0)) - eye_space_pos;
        float3 ddx2 = eye_space_pos - GetEyeSpacePos(screen_space_pos - float2(inv_res.x, 0));
        ddx = abs(ddx.z) > abs(ddx2.z) ? ddx2 : ddx;
        float3 ddy = GetEyeSpacePos(screen_space_pos + float2(0, inv_res.y)) - eye_space_pos;
        float3 ddy2 = eye_space_pos - GetEyeSpacePos(screen_space_pos - float2(0, inv_res.y));
    ...
    {
        const float2 screen_space_pos = i.texcoord;

        float3 eye_space_pos;
        float3 eye_space_norm;
        float3 eye_space_view_vec;
        CalcEyeSpacePosNormViewVec(screen_space_pos, eye_space_pos, eye_space_norm, eye_space_view_vec);
        ...
        return eye_space_pos.z != -_FarClipPlane ? float4(c, saturate(tex2D(_MainTex, screen_space_pos).a)) : 0;
    }
